...
const ThankYou = () => {
    ...
    const examineeResults = () => {
        fetch(`${getApiBasicUrl}/examinee-justin-result?user_info_id=${location.state.userId}&question_subject_id=${location.state.subjectId}`, {
            headers: {
        ...
        <div className='container min-h-screen mx-auto flex flex-col justify-center'>
            <div class="flex gap-[75px]  items-center justify-center bg-[#5c4449] py-[102px] rounded-md shadow-lg w-[644px] mx-auto">
                ...
                <div className='text-white'>
                    ...
                    <p className='text-3xl font-bold text-white rounded-3xl bg-cyan-500 mt-5 p-2'>Correct Answer : {getExamDetail.total_right_ans}/{getExamDetail.total_questions} </p>
                </div>
            </div>
        </div>
    );
